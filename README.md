...
というのも原始関数と導関数の挙動を同時に見る際は、UXの観点から左右よりも上下の方が圧倒的に追いやすいと考えていたからです。
気になる結果ですが、上下に原始関数・導関数を表示し、X軸の動きに合わせて二つのグラフが同期して動く仕様まで完璧に再現してくれました。
その後実際に使ってみると、上下2段では1枚あたりの縦幅が狭くグラフの形状(特に対数関数のような漸近線を持つもの)が読み取りにくかったため、左右並びのレイアウトに変更しています。
...
| モード | 内容 |
|---|---|
| 微分 | f(x) と f'(x) を左右 2 面で同時表示。x が動くと両グラフの点・接線(黄)・破線が連動。f''(x) の重ね表示も可能 |
